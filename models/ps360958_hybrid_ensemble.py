...
class MyModel(BaseModel):
    ...
    def build_model(self):
        ...
        with tf.variable_scope("ensembler")as scope0:
            with tf.name_scope("ensembler"):
                inputs = tf.concat([signal_5, signal_6, signal_8], axis=-1)
                d1 = tf.layers.dense(inputs, 16, activation=tf.nn.relu, name="dense1")
                d2 = tf.layers.dense(d1, 16, activation=tf.nn.relu, name="dense2")
                d3 = tf.layers.dense(d2, 2, activation=tf.nn.relu, name="dense3")
        ...
        with tf.name_scope("loss"):
            ...
            self.train_step = tf.train.AdamOptimizer(self.config.learning_rate).minimize(self.loss,
                                                                                        ...
                                                                                        var_list=tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='ensembler'))
